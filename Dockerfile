...
FROM deps AS build
COPY . .
RUN pnpm --filter @pms/web build && pnpm --filter @pms/website build && pnpm --filter @pms/booking build
...
# --- Public booking site — book.pms.do ---
FROM base AS booking
ENV NODE_ENV=production
ENV HOST=0.0.0.0
ENV PORT=3000
COPY --from=build /app/apps/booking/.output /app/.output
EXPOSE 3000
HEALTHCHECK --interval=15s --timeout=5s --start-period=15s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:3000/').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"
CMD ["node", ".output/server/index.mjs"]
