...
if (forbidden.length > 0) {
  console.error('Booking app must not depend on PMS server packages:', forbidden)
  process.exit(1)
}
...
if (nuxt.includes('s-maxage=3600')) {
  console.error('Booking routes must not use marketing s-maxage=3600')
  process.exit(1)
}
if (/Access-Control-Allow-Origin['"]?\s*:\s*['"]\*/.test(nuxt)) {
  console.error('Booking CORS must not be *')
  process.exit(1)
}

console.log('booking isolation ok')
